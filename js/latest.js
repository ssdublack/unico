...
function getCompleteParams() {
  ...
  const utmSource = searchParamsFromUrl.get('utm_source');
  const utmCampaign = searchParamsFromUrl.get('utm_campaign');
  const utmMedium = searchParamsFromUrl.get('utm_medium');
  const utmContent = searchParamsFromUrl.get('utm_content');
  const utmTerm = searchParamsFromUrl.get('utm_term');

  const completeParams = new URLSearchParams();
  if (utmSource) completeParams.set('utm_source', utmSource);
  if (utmCampaign) completeParams.set('utm_campaign', utmCampaign);
  if (utmMedium) completeParams.set('utm_medium', utmMedium);
  if (utmContent) completeParams.set('utm_content', utmContent);
  if (utmTerm) completeParams.set('utm_term', utmTerm);
  ...
  completeParams.set('src', getParam('src'));
